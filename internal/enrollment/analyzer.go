...
func (a *analyzer) validateAcquireRef(ref, file, job string) {
	if !isSHA(ref) {
		a.add("mutable-acquire-ref", file, job)
		return
	}
	if a.requiredAcquireSHA != "" && !strings.EqualFold(ref, a.requiredAcquireSHA) {
		a.add("unapproved-acquire-ref", file, job)
	}
}
