...
test("opt-in acquire records post cleanup after an ambiguous queue insertion", async () => {
  let state = semaphoreState([semaphoreHolder("other/repo", "999", "editmode")]);
  let stateReads = 0;
  let puts = 0;
  await withTempFile(async (stateFile) => {
    await withActionEnv(
      { ...semaphoreActionEnv, GITHUB_STATE: stateFile },
      async () => {
        await withImmediateTimers(async () => {
          await withMockedFetch(async (url, options = {}) => {
            const parsed = new URL(url);
            ...
            if (parsed.pathname === SEMAPHORE_STATE_PATH) {
              ...
              stateReads++;
              if (stateReads > 1) return jsonResponse(400, { message: "stop after ambiguous queue write" });
              return base64Content(state, "before");
            }
            return jsonResponse(404, { message: `unexpected path ${parsed.pathname}` });
          }, () => assert.rejects(
            () => acquire(semaphoreConfig({ registerPostCleanup: true, timeoutMinutes: 10 })),
            /stop after ambiguous queue write/
          ));
        });
      }
    );
...
test("acquire periodically removes a superseded PR while capacity remains occupied", async () => {
  ...
  assert.equal(state.holders[0].holderId, waitingHolder.holderId, "unrelated holder must remain authoritative");
  assert.deepEqual(state.queue, []);
});
    ...
    await withActionEnv(
      ...
      async () => {
        await withMockedFetch(async (url) => {
          ...
          if (parsed.pathname.includes("/repos/o/r/")) {
            lockStateAccesses++;
          }
          return jsonResponse(404, { message: `unexpected path ${parsed.pathname}` });
        }, async () => {
...
test("schema 5 immediate incident cleans a preexisting exact queue identity only", async () => {
  const incident = accountIncident({ runnerId: "runner-c" });
  const unrelated = withRunner(semaphoreHolder("other/repo", "999", "editmode"), "runner-b");
  const queued = withRunner(semaphoreQueueEntry("owner/repo", "123", "playmode"), "runner-a");
  let state = accountHealthState([unrelated], [queued], [], incident);
  let writes = 0;
  await withActionEnv({ ...semaphoreActionEnv }, async () => {
    await withMockedFetch(async (url, options = {}) => {
      const parsed = new URL(url);
      ...
      if (parsed.pathname === SEMAPHORE_STATE_PATH) {
        if (options.method === "PUT") {
          state = JSON.parse(Buffer.from(JSON.parse(options.body).content, "base64").toString("utf8"));
          writes++;
          return jsonResponse(200, { content: { sha: "cleaned" } });
        }
        return base64Content(state, "blocked");
      }
      return jsonResponse(404, { message: `unexpected path ${parsed.pathname}` });
    }, () => assert.rejects(
      () => acquire(semaphoreConfig({ runnerId: "runner-a" })),
  ...
  assert.deepEqual(state.queue, []);
  assert.deepEqual(state.holders, [unrelated], "queue-only incident cleanup must preserve preexisting holders");
  assert.equal(state.activeIncident.incidentId, incident.incidentId);
});

test("schema 5 incident appearing during a wait removes the caller's exact queue identity", async () => {
  const unrelated = withRunner(semaphoreHolder("other/repo", "999", "editmode"), "runner-b");
  const incident = accountIncident({ runnerId: unrelated.runnerId });
  let state = accountHealthState([unrelated]);
  let writes = 0;
  await withTempFile(async (outputFile) => {
    await withActionEnv({ ...semaphoreActionEnv, GITHUB_OUTPUT: outputFile }, async () => {
      await withImmediateTimers(async () => {
        await withMockedFetch(async (url, options = {}) => {
          ...
          if (parsed.pathname === SEMAPHORE_STATE_PATH) {
            if (options.method === "PUT") {
              state = JSON.parse(Buffer.from(JSON.parse(options.body).content, "base64").toString("utf8"));
              writes++;
              if (writes === 1) state.activeIncident = incident;
  ...
  assert.equal(writes, 2, "queue insertion and exact queue cleanup must each use one CAS write");
  assert.deepEqual(state.queue, []);
  assert.deepEqual(state.holders, [unrelated]);
  assert.equal(state.activeIncident.incidentId, incident.incidentId);
});
...
test("schema 5 incident published after admission is retracted before activation", async () => {
  ...
  assert.equal(writes, 2, "admission and pre-activation retraction must each use one CAS write");
  assert.deepEqual(state.holders, []);
  assert.deepEqual(state.queue, []);
  assert.deepEqual(state.reservations, []);
  assert.equal(state.activeIncident.incidentId, incident.incidentId);
